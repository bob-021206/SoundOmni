...
def create_sparse_mask(document_lens, split_lens, attn_modes, device):
    ...
    # Pad tensors to multiple of 128 to avoid index out of bounds in flex_attention
    seq_len = len(full_and_noise_seq_id)
    pad_to_multiple = 128
    target_len = (seq_len + pad_to_multiple - 1) // pad_to_multiple * pad_to_multiple
    
    if target_len > seq_len:
        pad_len = target_len - seq_len
        full_and_noise_seq_id = torch.cat([full_and_noise_seq_id, torch.full((pad_len,), -2, device=device)])
        noise_seq_id = torch.cat([noise_seq_id, torch.full((pad_len,), -2, device=device)])
        document_id = torch.cat([document_id, torch.full((pad_len,), -1, device=device)])
...
def add_special_tokens(tokenizer):
    ...
    if '<|audio_start|>' not in all_special_tokens:
        new_tokens.append('<|audio_start|>')

    if '<|audio_end|>' not in all_special_tokens:
        new_tokens.append('<|audio_end|>')
    
    # Check for Qwen2-Audio native tokens
    # If they exist, we should probably map our start/end to them or ensure they are used?
    # For now, we just add our own.
    ...
    # Use Qwen2-Audio native tokens if available, otherwise use ours
    if '<|audio_bos|>' in tokenizer.get_vocab():
        start_of_audio = tokenizer.convert_tokens_to_ids('<|audio_bos|>')
    else:
        start_of_audio = tokenizer.convert_tokens_to_ids('<|audio_start|>')
        
    if '<|audio_eos|>' in tokenizer.get_vocab():
        end_of_audio = tokenizer.convert_tokens_to_ids('<|audio_eos|>')
    else:
        end_of_audio = tokenizer.convert_tokens_to_ids('<|audio_end|>')
