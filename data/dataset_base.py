...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            # Audio transform for T2A dataset
            if 'audio_transform_args' in dataset_args.keys():
                from .audio_transforms import AudioTransform
                audio_transform = AudioTransform(**dataset_args.pop('audio_transform_args'))
                dataset_args['audio_transform'] = audio_transform

            assert 'dataset_names' in dataset_args.keys()
            dataset_names = dataset_args.pop('dataset_names')
            dataset_args['data_dir_list'] = []
            
            # Initialize lists for T2A dataset
            if grouped_dataset_name == 't2a_pretrain':
                dataset_args['csv_path_list'] = []
                dataset_args['audio_dir_list'] = []
            ...
            for item in dataset_names:
                ...
                # Handle T2A dataset paths
                if grouped_dataset_name == 't2a_pretrain':
                    # Get split from YAML config or use default
                    split = dataset_args.get('split', meta_info.get('default_split', 'train'))
                    
                    # Build CSV path based on split
                    csv_base_dir = meta_info['csv_base_dir']
                    csv_filename = f"{split}.csv"
                    
                    # Handle relative paths (relative to text_audio directory)
                    if not os.path.isabs(csv_base_dir):
                        # Get the directory containing the data module
                        data_module_dir = os.path.dirname(os.path.abspath(__file__))
                        text_audio_dir = os.path.dirname(data_module_dir)
                        csv_base_dir = os.path.join(text_audio_dir, csv_base_dir)
                    
                    csv_path = os.path.join(csv_base_dir, csv_filename)
                    
                    # Handle audio_dir relative path
                    audio_dir = meta_info['audio_dir']
                    if not os.path.isabs(audio_dir):
                        data_module_dir = os.path.dirname(os.path.abspath(__file__))
                        text_audio_dir = os.path.dirname(data_module_dir)
                        audio_dir = os.path.join(text_audio_dir, audio_dir)
                    
                    dataset_args['csv_path_list'].append(csv_path)
                    dataset_args['audio_dir_list'].append(audio_dir)
    ...
    def set_sequence_status(self):
        sequence_status = dict(
            ...
            # Audio-related lists
            audio_feature_tensors       = list(),  # Store audio features
            packed_audio_gen_position_ids = list(),  # Position IDs for audio
            audio_feature_shapes        = list(),  # Audio feature shapes
            packed_audio_gen_token_indexes = list(),  # Audio token indexes
            packed_audio_timesteps      = list(),
            audio_mse_loss_indexes      = list(),
    ...
    def to_tensor(self, sequence_status):
        ...
        # if the model has audio features (for audio generation)
        if len(sequence_status['audio_feature_tensors']) > 0:
            audio_features = sequence_status.pop('audio_feature_tensors')
            # Pad audio waveforms to max length
            # audio_features shape: list of [C, T] tensors
            audio_shapes = [item.shape for item in audio_features]  # [(C1, T1), (C2, T2), ...]
            max_C = max(shape[0] for shape in audio_shapes)  # Max channels
            max_T = max(shape[1] for shape in audio_shapes)  # Max time samples
            
            padded_audio = torch.zeros(size=(len(audio_features), max_C, max_T))
            for i, audio_feature in enumerate(audio_features):
                C, T = audio_feature.shape
                padded_audio[i, :C, :T] = audio_feature

            data['padded_audio_features'] = padded_audio
            data['audio_feature_shapes'] = sequence_status['audio_feature_shapes']
            data['packed_audio_gen_position_ids'] = torch.cat([torch.tensor(pos_ids) for pos_ids in sequence_status['packed_audio_gen_position_ids']], dim=0)
            data['packed_audio_gen_token_indexes'] = torch.tensor(sequence_status['packed_audio_gen_token_indexes'])
        ...
        if len(sequence_status['packed_audio_timesteps']) > 0:
            data['packed_audio_timesteps'] = torch.tensor(sequence_status['packed_audio_timesteps'])
            data['audio_mse_loss_indexes'] = torch.tensor(sequence_status['audio_mse_loss_indexes'])
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        audio_feature_list = sample.get('audio_feature_list', [])  # Audio features
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
            ...
            elif item['type'] == 'audio_latent':
                # Handle audio latent (similar to vae_image)
                audio_feature = audio_feature_list.pop(0)
                if item['enable_cfg'] == 1 and random.random() < self.data_config.text_cond_dropout_prob:
                    curr_rope_id += 1
                    continue

                # add a <|startofaudio|> token
                sequence_status['packed_text_ids'].append(self.start_of_audio)
                sequence_status['packed_text_indexes'].append(curr)
                curr += 1
                curr_split_len += 1

                # preprocess audio waveform
                sequence_status['audio_feature_tensors'].append(audio_feature)
                C, T = audio_feature.shape  # [C, T] where C=channels, T=time samples
                
                # Store entire waveform as one unit
                # The actual tokenization will be done by Audio Encoder during training
                sequence_status['audio_feature_shapes'].append((C, T))
                
                # Calculate actual number of tokens based on downsample rate (2048)
                num_audio_tokens = T // 2048
                if num_audio_tokens == 0:
                    num_audio_tokens = 1

                audio_position_ids = list(range(num_audio_tokens))
                sequence_status['packed_audio_gen_position_ids'].append(audio_position_ids)
                sequence_status['packed_audio_gen_token_indexes'].extend(range(curr, curr + num_audio_tokens))
                
                if item['loss'] == 1:
                    sequence_status['audio_mse_loss_indexes'].extend(range(curr, curr + num_audio_tokens))
                    # Always sample a timestep for audio flow-matching.
                    # (Will be squashed to (0,1) by sigmoid in the model.)
                    timestep = np.random.randn()
                else:
                    timestep = float('-inf')
                
                sequence_status['packed_audio_timesteps'].extend([timestep] * num_audio_tokens)

                curr += num_audio_tokens
                curr_split_len += num_audio_tokens

                # add a <|endofaudio|> token
                sequence_status['packed_text_ids'].append(self.end_of_audio)
                sequence_status['packed_text_indexes'].append(curr)
                # <|endofaudio|> may have loss
                if item['special_token_loss'] == 1:
                    sequence_status['ce_loss_indexes'].append(curr)
                    sequence_status['ce_loss_weights'].append(1.0)
                    sequence_status['packed_label_ids'].append(item['special_token_label'])
                ...
                # update sequence status
                attn_modes.append("full")
                sequence_status['packed_position_ids'].extend([curr_rope_id] * (num_audio_tokens + 2))
                curr_rope_id += 1
...
class SimpleCustomBatch:
    def __init__(self, batch):
        ...
        # Audio features
        if "padded_audio_features" in data.keys():
            self.padded_audio_features = data["padded_audio_features"]
            self.audio_feature_shapes = data["audio_feature_shapes"]
            self.packed_audio_gen_position_ids = data["packed_audio_gen_position_ids"]
            self.packed_audio_gen_token_indexes = data["packed_audio_gen_token_indexes"]

        if "packed_timesteps" in data.keys():
            self.packed_timesteps = data["packed_timesteps"]
            self.mse_loss_indexes = data["mse_loss_indexes"]
        
        if "packed_audio_timesteps" in data.keys():
            self.packed_audio_timesteps = data["packed_audio_timesteps"]
            self.audio_mse_loss_indexes = data["audio_mse_loss_indexes"]
    ...
    def pin_memory(self):
        ...
        if hasattr(self, 'padded_audio_features'):
            self.padded_audio_features = self.padded_audio_features.pin_memory()
            self.packed_audio_gen_token_indexes = self.packed_audio_gen_token_indexes.pin_memory()
            self.packed_audio_gen_position_ids = self.packed_audio_gen_position_ids.pin_memory()
        ...
        if hasattr(self, 'packed_audio_timesteps'):
            self.packed_audio_timesteps = self.packed_audio_timesteps.pin_memory()
            self.audio_mse_loss_indexes = self.audio_mse_loss_indexes.pin_memory()
    ...
    def cuda(self, device):
        ...
        if hasattr(self, 'padded_audio_features'):
            self.padded_audio_features = self.padded_audio_features.to(device)
            self.packed_audio_gen_token_indexes = self.packed_audio_gen_token_indexes.to(device)
            self.packed_audio_gen_position_ids = self.packed_audio_gen_position_ids.to(device)
        ...
        if hasattr(self, 'packed_audio_timesteps'):
            self.packed_audio_timesteps = self.packed_audio_timesteps.to(device)
            self.audio_mse_loss_indexes = self.audio_mse_loss_indexes.to(device)
    ...
    def to_dict(self):
        ...
        if hasattr(self, 'padded_audio_features'):
            data['padded_audio_features'] = self.padded_audio_features
            data['audio_feature_shapes'] = self.audio_feature_shapes
            data['packed_audio_gen_position_ids'] = self.packed_audio_gen_position_ids
            data['packed_audio_gen_token_indexes'] = self.packed_audio_gen_token_indexes
        ...
        if hasattr(self, 'packed_audio_timesteps'):
            data['packed_audio_timesteps'] = self.packed_audio_timesteps
            data['audio_mse_loss_indexes'] = self.audio_mse_loss_indexes
