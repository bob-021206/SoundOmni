...
class FSDPCheckpoint:
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        if resume_from is not None and os.path.exists(resume_from):
            ...
            logger.info(f"Load state dict result: {msg}")
            if len(msg.missing_keys) > 0:
                logger.warning(f"Missing keys: {msg.missing_keys[:20]} ... (total {len(msg.missing_keys)})")
                # Check for bottleneck keys
                bottleneck_missing = [k for k in msg.missing_keys if "mlp_moe_gen" in k]
                if len(bottleneck_missing) > 0:
                    logger.error(f"CRITICAL: {len(bottleneck_missing)} Bottleneck keys are missing from checkpoint! Example: {bottleneck_missing[0]}")
    ...
    def try_load_train_state(resume_from, optimizer, scheduler, fsdp_config):
        if resume_from is not None and os.path.exists(resume_from):
            ...
            try:
                ...
                print(f"Successfully loaded optimizer state from {optimizer_state_dict_path}")
                # Debug: check if step is restored
                if len(optimizer.param_groups) > 0:
                    first_group = optimizer.param_groups[0]
                    # Check first param state
                    if len(optimizer.state) > 0:
                        first_param = next(iter(optimizer.state.values()))
                        if 'step' in first_param:
                            print(f"Optimizer step restored to: {first_param['step']}")
                        else:
                            print("Optimizer state loaded but 'step' not found in first param state!")
                    else:
                        print("Optimizer state loaded but state dict is empty!")
            ...
            print(f"Successfully loaded scheduler state from {scheduler_state_dict_path}")
